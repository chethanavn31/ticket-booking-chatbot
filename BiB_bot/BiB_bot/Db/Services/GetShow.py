from MysqlConnector import createCon,closeCon
from Constants import dbConnError
from MysqlQueries import getShowTimeQuery

import sys
#import MySQLdb
import json
import os

def getShow(payload):
    output={}
    status='E'
    message=''
    errCode=3
    conn='E'
    cur=None
    showData=[]

    try:
        #connect to db
        conn=createCon()
        if conn=='E':
            errCode=500
            raise Exception(dbConnError)

        cur = conn.cursor()

        #capture data
        jDecode = json.loads(payload)

        theatreId = jDecode['theatreId']
        
        #execute query
        inputVal = (theatreId,)
        cur.execute(getShowTimeQuery,inputVal)

        showDetails = cur.fetchall()

        for val in showDetails:
            obj={
                'type':val[0]
            }
            showData.append(obj)

        status='S'
        message='Success'
        errCode=0

    except Exception as exce:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        status='E'
        if errCode==3:
            errCode=1005
        message = str(exce)+' '+str(exc_tb.tb_lineno)

    
        if conn!='E':
            conn.rollback()
        
    finally:
        if conn!='E':
            cur.close()
            closeCon(conn)

    output={
        'data':{
            'status':status,
            'message':message,
            'errorCode':errCode,
            'showDetails':showData
        }
    }
    output=json.dumps(output)
    print(output)

if __name__ == "__main__":
    getShow(sys.argv[1])